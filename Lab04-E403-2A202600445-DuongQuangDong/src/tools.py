...
def format_price(price: int) -> str:
    """Format giá tiền thêm dấu chấm phân cách, ví dụ: 1.450.000đ"""
...
def search_flights(origin: str, destination: str) -> str:
    ...
    """
    # Tra cứu với tuple (origin, destination)
    flights = FLIGHTS_DB.get((origin, destination))
    
    # Thử tra ngược lại nếu chiều thuận không có
    if flights is None:
    ...
    result_lines = [f"Danh sách chuyến bay giữa {origin} và {destination}:"]
    for idx, f in enumerate(flights, 1):
        price_str = format_price(f['price'])
        result_lines.append(f"{idx}. {f['airline']} | Đi: {f['departure']} - Đến: {f['arrival']} | Giá: {price_str} ({f['class']})")
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result_lines = [f"Danh sách khách sạn tại {city} (Giá <= {format_price(max_price_per_night)}/đêm):"]
    for idx, h in enumerate(filtered_hotels, 1):
        price_str = format_price(h['price_per_night'])
        result_lines.append(f"{idx}. {h['name']} ({h['stars']} sao) | Khu vực: {h['area']} | Rating: {h['rating']} | Giá: {price_str}/đêm")
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        items = [i.strip() for i in expenses.split(',') if i.strip()]
        for item in items:
            name, amount_str = item.rsplit(':', 1)
            name = name.strip().replace('_', ' ').capitalize()
            expense_dict[name] = int(amount_str.strip())
        ...
        result_lines.append("---")
        result_lines.append(f"Tổng chi: {format_price(total_expense)}")
        result_lines.append(f"Ngân sách: {format_price(total_budget)}")
        
        if remaining >= 0:
            result_lines.append(f"Còn lại: {format_price(remaining)}")
        else:
            result_lines.append(f"Còn lại: {format_price(remaining)}")
            result_lines.append(f"Vượt ngân sách {format_price(abs(remaining))}! Cần điều chỉnh.")
            
        return "\n".join(result_lines)
    except Exception as e:
        return f"Lỗi format chuỗi expenses. Vui lòng sử dụng định dạng 'tên_khoản:số_tiền' cách nhau bằng dấu phẩy. Lỗi cụ thể: {str(e)}"
